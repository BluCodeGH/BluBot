#An asyncio version of python's code module written by BluCode.
...
class InteractiveInterpreter:
  ...
  async def runsource(self, source, filename="<input>", symbol="single", ret=False):
    ...
    try:
      ...
      return False, "DONTRETURN"

    if code is None:
      # Case 2
      return True, "DONTRETURN"

    # Case 3
    res = await self.runcode(code)
    if ret:
      return False, res
    else:
      return False, "DONTRETURN"
...
class InteractiveConsole(InteractiveInterpreter):
  ...
  async def interact(self, banner=None, exitmsg=None):
    ...
    if banner is None:
      await self.write("Python %s on %s\n%s\n(%s)\n" %
    ...
    while 1:
      try:
        ...
        try:
          ...
          more, res = await self.push(line, True)
          if res != "DONTRETURN":
            await self.write(res)
  ...
  async def push(self, line, ret=False):
    ...
    source = "\n".join(self.buffer)
    more, res = await self.runsource(source, self.filename, ret=True)
    if not more:
      self.resetbuffer()
    if ret:
      return more, res
